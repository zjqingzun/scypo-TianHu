...
cJSON *__loadErrorMessages(const char *_filePath) {
    ...
    if (!_file) {
        fprintf(stderr, "!(app/core/err.c) Cannot open file: %s\n", _filePath);
        return NULL;
    }
    ...
    if (!_fileContent) {
        fprintf(stderr, "!(app/core/err.c) Memory allocation failed for reading file\n");
        fclose(_file);
        return NULL;
    }
    ...
    if (!_root) {
        fprintf(stderr, "!(app/core/err.c) Error parsing JSON: %s\n", cJSON_GetErrorPtr());
        return NULL;
    }
...
void __getErrorMessage(const char *_errorCode, cJSON *_errorData) {
    if (!_errorData) {
        printf("!(app/core/err.c) Error data is not available\n");
        return;
    }
    ...
    if (!_errorObj || !cJSON_IsObject(_errorObj)) {
        printf("!(app/core/err.c) Error Code: %s\n", _errorCode);
